...
public class Robot_NN_law_pro_2_0_2 extends AdvancedRobot {
    ...
    private final double exploratory_rate = 0.1;//exploratory_rate

    /*--------temp-parameter String & double-------*/
    final int state_num = 7;
    final int action_num = 7;
    private double[] state_NN;
    private double[][] states ;
    private double[][] values ;
    private double[] state_action_pre = new double[state_num+action_num];     //7 input, 6 output
    private double[] state_action_for = new double[state_num+action_num];     //7 input, 6 output
    private double[] Q_values_pre = new double[action_num];
    private double[] Q_values_for = new double[action_num];
    ...
    File Win_rate = new File("/Users/Lawrence Li/robocode/robots/Spurs/LUT_robot.data/Win_rate_pro_22.txt");

    File state_value_file = new File("/Users/Lawrence Li/robocode/robots/Spurs/LUT_robot.data/State_Values_pro_22.txt");
    ...
    public Robot_NN_law_pro_2_0_2() throws IOException {
    }
    ...
    public double [][] queue_to_state(CircularDoubleArrayQueue q){
        Object[] le = q.toArray();
        double[][] s =new double[le.length][state_num];
    ...
    public double [][] queue_to_value(CircularDoubleArrayQueue q){
        Object[] le = q.toArray();
        double[][] v =new double[le.length][action_num];

        for (int i = 0; i < le.length; i++) {
            for (int j = state_num; j < action_num+state_num; j++) {
                v[i][j-state_num] = q.get(i)[j];
            }
        }
        return v;
    }
    ...
    public void run(){
        ...
        while (true) {
            ...
            System.out.println(next_Action+"be");
            //Take action in the pre-set method
            action7(next_Action);
            //Check where I am
            turnRadarLeft(360);
            System.out.println(next_Action+"af");
